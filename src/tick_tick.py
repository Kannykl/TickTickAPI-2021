...
def process_response_code(action: str, code: int):
    ...
    """
    if code != 200:
        raise Exception(f"Failed to {action}, error code is {code}")
...
class TickAPI:
    def __init__(self, username: str, passwd: str):
        ...
        request = self.session.post(
            f"{self.base_url}/user/signon?wc=true",
            json={
                "username": f"{username}",
                "password": f"{passwd}",
            },
        )
        process_response_code('login', request.status_code)
    ...
    def get_projects_id(self) -> List[str]:
        ...
        process_response_code('get projects', request.status_code)
        data = request.json()
        for i in data:
            id_ = i["id"]
            projects.append(id_)
    ...
    def get_tasks_by_project_id(self, project_id: str) -> list:
        ...
        request = self.session.get(url)
        process_response_code('get project tasks', request.status_code)
        data = request.json()
    ...
    def get_completed_tasks_by_project_id(
        ...
        request = self.session.get(url)
        process_response_code('get completed tasks', request.status_code)
        return request.json()
    ...
    def get_tags(self) -> List[dict]:
        ...
        url = f"{self.base_url}/batch/check/1"
        request = self.session.get(url)
        process_response_code('get tags', request.status_code)
    ...
    def get_project_by_id(self, project_id: str) -> dict:
        ...
        url = f"{self.base_url}/projects"
        request = self.session.get(url)
        process_response_code('get the task list', request.status_code)
    ...
    def get_project_by_name(self, name: str) -> dict:
        ...
        url = f"{self.base_url}/projects"
        request = self.session.get(url)
        process_response_code('get the project list', request.status_code)
        projects = request.json()
        for project in projects:
    ...
    def add_task(
        ...
        process_response_code('create task', request.status_code)
        return name
    ...
    def add_tag(self, tag_name: str, colour: str = None, parent: str = None) -> str:
        ...
        request = self.session.post(url, json=data)
        process_response_code('create tag', request.status_code)
        return tag_name
    ...
    def add_project(
        ...
        request = self.session.post(url, json=data)
        process_response_code('create project', request.status_code)
    ...
    def delete_tag(self, name: str) -> str:
        ...
        process_response_code('remove this tag', request.status_code)
        return name
    ...
    def delete_project(self, name: str) -> str:
        ...
        request = self.session.post(url, json=data)
        process_response_code('delete project', request.status_code)
        return project_id
    ...
    def delete_task(self, task_title: str, project_id: str) -> str:
        ...
        request = self.session.post(url, json=data)
        process_response_code('delete task', request.status_code)
        return task[0]["id"]
    ...
    def empty_trash(self):
        ...
        request = self.session.delete(url)
        process_response_code('empty trash', request.status_code)
        return 0
