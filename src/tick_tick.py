...
class TickAPI:
    def __init__(self, username: str, passwd: str):
        ...
        url = self._create_url("/user/signon", {"wc": "true"})
    ...
    def _create_url(self, action: str, args: dict = None) -> str:
        """Create url
        Args: action(str): function of url.
            args (str): args of url endpoint.
        Returns:
            Str: API url.
        """
        api_url = furl(self.base_url)
        api_url /= action
        if args:
            for key, value in args.items():
                api_url.add({key: value})
        return api_url.url
    ...
    def get_projects_id(self) -> List[str]:
        ...
        if request.status_code == 200:
            ...
            raise Exception(
                f"Failed to get projects, error code is {request.status_code}"
            )
        return projects
    ...
    def get_tasks_by_project_id(self, project_id: str) -> list:
        ...
        """
        tasks_list = list()
        url = self._create_url("batch/check/1")
        request = self.session.get(url)
        if request.status_code != 200:
    ...
    def get_completed_tasks_by_project_id(
        ...
        """
        url = self._create_url(f"project/{project_id}/completed", {"limit": count})
        request = self.session.get(url)
        if request.status_code != 200:
            raise Exception(
                f"Failed to get completed tasks, error code is {request.status_code}"
            )
        return request.json()
    ...
    def get_project_by_id(self, project_id: str) -> dict:
        ...
        if request.status_code != 200:
            raise Exception(
                f"Failed to get the task list, error code is {request.status_code}"
            )
    ...
    def add_task(
        self, name: str, project_id: str, content: str = "", tag: list = None
    ) -> str:
        ...
        if tag is None:
            tag = []
        url = self._create_url("batch/task")
        ...
        if request.status_code != 200:
            raise Exception(
                f"Failed to create task, error code is {request.status_code}"
            )
        return name
    ...
    def add_tag(self, tag_name: str, colour: str = None, parent: str = None) -> str:
        ...
        if colour and colour.lower() not in self.colours.keys():
            raise KeyError(f"So far, {colour} colour is not supported..")
        url = self._create_url("batch/tag")
        data = {
            "add": [
                {
        ...
        if request.status_code != 200:
            raise Exception(
                f"Failed to create tag, error code is {request.status_code}"
            )
        return tag_name

    def add_project(
        self, project_title: str, colour: str = "", type_of_project: str = "TASK"
    ) -> str:
        ...
        if request.status_code != 200:
            raise Exception(
                f"Failed to create project, error code is {request.status_code}"
            )
    ...
    def delete_tag(self, name: str) -> str:
        ...
        url = self._create_url("tag", {"name": name})
        request = self.session.delete(url)
        if request.status_code != 200:
            raise ValueError(
                f"Unable to remove this tag, error code is {request.status_code}"
            )
        return name
    ...
    def delete_project(self, name: str) -> str:
        ...
        url = self._create_url("batch/project")
        data = {"add": [], "update": [], "delete": [project_id]}
        request = self.session.post(url, json=data)
        if request.status_code != 200:
            raise Exception(
                f"Failed to delete project, error code is {request.status_code}"
            )
        return project_id
    ...
    def delete_task(self, task_title: str, project_id: str) -> str:
        ...
        url = self._create_url("batch/task")
        data = {"delete": [{"taskId": f"{task[0]['id']}", "projectId": project_id}]}
        request = self.session.post(url, json=data)
        if request.status_code != 200:
            raise Exception(
                f"Failed to delete task, error code is {request.status_code}"
            )
        return task[0]["id"]
    ...
    def empty_trash(self):
        ...
        Returns:
            int: 0
        """
        url = self._create_url("trash/empty")
        request = self.session.delete(url)
        if request.status_code != 200:
            raise Exception(
                f"Failed to empty trash, error code is {request.status_code}"
            )
        return 0
