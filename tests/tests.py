...
def test_successful_get_project_by_name(
    authenticated_session: TickAPI, test_projects, test_project_names
):
...
def test_get_projects_id(authenticated_session: TickAPI, test_projects):
    ...
    assert (
        projects_id[0]
        == authenticated_session.get_project_by_name(test_projects[0])["id"]
    )
    assert (
        projects_id[1]
        == authenticated_session.get_project_by_name(test_projects[1])["id"]
    )
    assert (
        projects_id[2]
        == authenticated_session.get_project_by_name(test_projects[2])["id"]
    )
...
def test_add_task(authenticated_session: TickAPI, test_tasks):
    ...
    task1 = authenticated_session.add_task(
        "OnlyOneTask", projects_id[0], "TestContent1"
    )
    task2 = authenticated_session.add_task(
        "SecondAwesomeTask", projects_id[1], "TestContent2"
    )
...
def test_add_task_with_tag(authenticated_session: TickAPI, test_tags, test_projects):
    projects_id = authenticated_session.get_projects_id()
    my_task = authenticated_session.add_task(
        "UniqueTask", projects_id[1], "awesomecontent", [test_tags[0]]
    )
...
def test_add_task_failed_duplicate_title(authenticated_session: TickAPI, test_projects):
    projects_id = authenticated_session.get_projects_id()
    authenticated_session.add_task(
        "DuplicateName",
        projects_id[1],
        "awesomecontent",
    )
...
def test_get_projects_by_colour(
    authenticated_session: TickAPI, test_projects, test_project_colours
):
    red_project = authenticated_session.get_projects_by_colour(test_project_colours[0])
    assert red_project[0]["name"] == test_projects[0]
    purple_project = authenticated_session.get_projects_by_colour(
        test_project_colours[1]
    )
    assert purple_project[0]["name"] == test_projects[1]
    turquoise_project = authenticated_session.get_projects_by_colour(
        test_project_colours[2]
    )
...
def test_delete_task(authenticated_session: TickAPI, test_projects, test_tags):
    ...
    authenticated_session.add_task(
        "TESTTASK", projects[0], "content", [tags[0]["name"]]
    )
